...
import { listLatestPosts } from "@/lib/community/hotPosts";
...
export async function generateMetadata({
  ...
  return {
    title: isKo ? "최신 게시글 — KickVista" : "Latest Posts — KickVista",
    description: isKo
      ? "커뮤니티의 가장 최신 축구 토론을 확인하세요."
      : "The most recent football discussions from the community.",
  };
}
...
export default async function LatestPostsPage({
  ...
  const { posts, total, hasMore } = await listLatestPosts({
  ...
  const tx = {
    title:         isKo ? "최신 게시글" : "Latest Posts",
    subtitle:      isKo ? "커뮤니티의 최신 축구 토론" : "The most recent football discussions",
    allCategories: isKo ? "전체" : "All",
    back:          isKo ? "← 커뮤니티로" : "← Community",
    hot:           isKo ? "🔥 인기 보기 →" : "🔥 View Hot →",
    pinned:        isKo ? "📌 고정 게시글" : "📌 Pinned",
    allPosts:      isKo ? "모든 게시글" : "All Posts",
    noPosts:       isKo ? "아직 게시글이 없습니다 — 첫 번째로 작성해보세요!" : "No posts yet — be the first!",
  ...
  return (
    <main style={{ background: "#0d1117", minHeight: "100vh" }}>
      ...
      <div
        style={{
          background: "radial-gradient(ellipse 80% 60% at 50% 0%, rgba(59,130,246,0.06) 0%, transparent 70%), linear-gradient(180deg, #0f1923 0%, #0d1117 100%)",
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                <span style={{ width: 3, height: 24, borderRadius: 2, backgroundColor: "#3b82f6", flexShrink: 0 }} />
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center", paddingTop: 24 }}>
              <a
                href={`/${loc}/community/hot${activeCategory ? `?category=${activeCategory}` : ""}`}
                style={{
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5">
        <AdSlot slotId="community-latest-top" size="leaderboard" />
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16 flex flex-col gap-6">
        ...
        <div style={{ display: "flex", gap: 6, overflowX: "auto", scrollbarWidth: "none", paddingBottom: 2 }}>
          ...
          {ALL_CATEGORIES.map((cat) => {
            ...
            return (
              <a
                key={cat}
                href={`/${loc}/community/latest?category=${cat}`}
                style={{
        ...
        {(activePage > 1 || hasMore) && (
          <div style={{ display: "flex", justifyContent: "center", gap: 8, paddingTop: 8 }}>
            {activePage > 1 && (
              <a
                href={`/${loc}/community/latest?${new URLSearchParams({
